...
The wrappers at `Resources/bin/claude` and `Resources/bin/codex` are PATH-scoped inside c11 terminals. They do not write to tenant-owned persistent config such as `~/.claude`, `~/.codex`, project `.codex` files, or shell rc files. Codex's wrapper may write a c11-owned CODEX_HOME overlay under c11 Application Support and point the child Codex process at that overlay for `--profile-v2 c11`; auth/state entries from the operator's real Codex home are linked into the overlay for continuity, while `config.toml` is copied so hook-trust writes stay c11-owned. The host remains unopinionated about the terminal: c11 provides the surface, the socket, the wrapper session-resume rail, and the skill file. For TUIs without a wrapper (kimi, opencode, etc.), skill-driven self-reporting is still how status gets populated.
